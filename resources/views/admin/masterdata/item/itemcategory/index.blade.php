    ...
    <div class="toolbar py-5 py-lg-5" id="kt_toolbar">
        <div id="kt_toolbar_container" class="container-xxl d-flex flex-stack flex-wrap">
            <div class="page-title d-flex flex-column me-3">
                <h1 class="d-flex text-dark fw-bolder my-1 fs-3">Item Category</h1>
        ...
        <div class="content flex-row-fluid" id="kt_content">
            <div class="card bg-white">
                <div class="card-header border-0 pt-6">
                    <div class="card-title align-items-start flex-column">
                        ...
                        <div class="d-flex align-items-center position-relative my-1">
                            ...
                            <!--end::Svg Icon-->
                            <input type="text" id="searchitemCategoryTable"
                ...
                <div class="card-body pt-0">
                    <table class="table align-middle table-row-dashed fs-6 gy-5" id="itemCategoryTable">
                        <thead>
...
@section('js')
    <script>
        function addModal() {
            ...
            $.get("{{ url('/admin/masterdata/item/category/create') }}", {}, function(data, status) {
                $('#kontennya').html(data)
                $('#mainmodal').modal('toggle')
                $('#loading-add').html(
                    '<button type="button" class="btn btn-primary me-3" id="add-btn" onclick="addModal()">Add Category</button>'
                    )
            })
        }
        ...
        function editModal(id) {
            $('#loading-add').html(
                '<div class="spinner-grow text-success" role="status"><span class="sr-only"></span></div>')
            $.get("{{ url('/admin/masterdata/item/category/edit') }}/" + id, {}, function(data, status) {
                $('#kontennya').html(data)
                $('#mainmodal').modal('toggle')
                $('#loading-add').html(
                    '<button type="button" class="btn btn-primary me-3" id="add-btn" onclick="addModal()">Add Category</button>'
        ...
        var itemCategoryTable = $('#itemCategoryTable').DataTable({
            ...
            processing: true,
            ajax: {
                url: "{{ url('/admin/masterdata/item/category/list') }}"
            },
            columns: [{
                    data: 'DT_RowIndex',
                    searchable: false
        ...
        $('#searchitemCategoryTable').keyup(function() {
            itemCategoryTable.search($(this).val()).draw()
        });
